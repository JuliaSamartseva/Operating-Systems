...
public class Kernel extends Thread {
  ...
  private static WorkingSet workingSet = new WorkingSet(10);
          ...
          if (line.startsWith("memset")) {
            ...
            while (st.hasMoreTokens()) {
              ...
              if ((0 > id || id > virtPageNum)
                  || (-1 > physical || physical > ((virtPageNum - 1) / 2))) {
        ...
        if (line.startsWith("READ") || line.startsWith("WRITE")) {
          ...
          if (tmp.startsWith("random")) {
            ...
            if (0 > addr || addr > address_limit) {
              System.out.println(
                  "MemoryManagement: " + addr + ", Address out of range in " + commands);
              System.exit(-1);
            }
  ...
  public void step() {
    ...
    if (instruct.inst.startsWith("READ")) {
      Page page =
          (Page) memVector.elementAt(Virtual2Physical.pageNum(instruct.addr, virtPageNum, block));
      if (page.physical == -1) {
        if (doFileLog) {
        ...
        if (doStdoutLog) {
          System.out.println(
              "READ " + Long.toString(instruct.addr, addressradix) + " ... page fault");
        }
    ...
    if (instruct.inst.startsWith("WRITE")) {
      ...
      if (page.physical == -1) {
        ...
        if (doStdoutLog) {
          System.out.println(
              "WRITE " + Long.toString(instruct.addr, addressradix) + " ... page fault");
        }
        PageFault.replacePage(
            memVector,
            virtPageNum,
